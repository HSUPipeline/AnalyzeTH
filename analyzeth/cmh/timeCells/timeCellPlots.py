...
def plot_FR(firing_rates, times, ax=None, title='', date='', SAVEFIG=False):

    # -- CATCH NO SPIKES --
    # if sum(firing_rates) == 0:
    #     print ('\n -- NO SPIKES FOUND --')
    #     print (title)
    #     return ax
    ...
    # FR Line Plot
    ax.plot(np.array(times)/1e3, firing_rates) 
    ax.set_ylim(0, max(firing_rates)+1)
